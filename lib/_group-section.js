...
function eventsReport(data,limit=100){
  ensureRoot(data);
  return {
    ok:true,
    events:data.groupEvents.slice(0,limit),
    backups:data.groupBackups.slice(0,Math.min(50,limit)),
    counters:{
      groups:Object.keys(data.groupSections||{}).length,
      events:data.groupEvents.length,
      backups:data.groupBackups.length
    }
  };
}
