...
function addEvent(data,type,siteId,groupKey,section,before,after,user,backupId,extra={}){
...
function updateSection(data,siteId,groupKey,section,value,opts={}){
  ...
  if(opts.requireRevision===true && opts.force!==true){
    const currentRevision=previous?.revision||0;
    const baseRevision=parseRevision(opts.baseRevision);
    if(!Number.isFinite(baseRevision)){
      throw new Error('Serverstand unklar: Bitte diese Gruppe zuerst neu vom Server laden, dann nochmals speichern.');
    }
    if(currentRevision!==baseRevision){
      const err=new Error('Speichern gestoppt: Auf dem Server ist bereits ein neuerer Stand vorhanden (Server Rev '+currentRevision+', dein Stand Rev '+baseRevision+'). Bitte zuerst neu laden.');
      err.code='GROUP_REVISION_CONFLICT';
      throw err;
    }
  }
...
function findBackup(data,backupId){
  ensureRoot(data);
  const id=safe(backupId);
  if(!id) throw new Error('Backup-ID fehlt.');
  const b=(Array.isArray(data.groupBackups)?data.groupBackups:[]).find(x=>safe(x&&x.backupId)===id);
  if(!b) throw new Error('Backup nicht gefunden: '+id);
  if(!b.previous || typeof b.previous!=='object') throw new Error('Backup enthält keinen wiederherstellbaren Stand.');
  return b;
}
function restoreBackup(data,siteId,groupKey,section,backupId,opts={}){
  const b=findBackup(data,backupId);
  const bSite=slug(b.siteId);
  const bGroup=slug(b.groupKey);
  const bSection=sectionName(b.section||section||'state');
  if(siteId && slug(siteId)!==bSite) throw new Error('Backup gehört zu einem anderen Haus/Standort.');
  if(groupKey && slug(groupKey)!==bGroup) throw new Error('Backup gehört zu einer anderen Gruppe.');
  if(section && sectionName(section)!==bSection) throw new Error('Backup gehört zu einem anderen Bereich.');

  const ctx=currentSection(data,bSite,bGroup,bSection);
  const group=ctx.group;
  const previous=ctx.current||null;
  const user=opts.user||{};
  const safetyBackupId=backupSection(data,bSite,bGroup,bSection,previous,user);
  const restoredValue=cloneValue(b.previous.value ?? defaultValueForSection(bSection));
  const next={
    value:restoredValue,
    revision:(previous?.revision||0)+1,
    hash:hash(restoredValue),
    updatedAt:nowIso(),
    updatedBy:user,
    restoredFrom:{
      backupId:b.backupId,
      revision:b.previous.revision||0,
      hash:b.previous.hash||'',
      createdAt:b.createdAt||'',
      safetyBackupId:safetyBackupId||''
    }
  };
  group.sections[bSection]=next;
  group.updatedAt=next.updatedAt;
  group.siteId=bSite;
  group.groupKey=bGroup;

  const type = bSection==='state' ? 'group.state.restored' :
               bSection==='employees' ? 'group.employees.restored' :
               bSection==='plan' ? 'group.plan.restored' :
               bSection==='duties' ? 'group.duties.restored' :
               bSection==='wishes' ? 'group.wishes.restored' : 'group.restored';
  const ev=addEvent(data,type,bSite,bGroup,bSection,previous,next,user,b.backupId,{
    restoredFromBackupId:b.backupId,
    restoredFromRevision:b.previous.revision||0,
    restoredFromHash:b.previous.hash||'',
    safetyBackupId:safetyBackupId||''
  });
  return Object.assign(publicSection(data,bSite,bGroup,bSection), {event:ev, backupId:b.backupId, safetyBackupId, restoredFrom:next.restoredFrom});
}
...
module.exports={
  ...
  ensureRoot,publicSection,updateSection,restoreBackup,extractValue,legacyPreview,eventsReport,userFromBody
};
